...
void matrix_t<T>::multiply(const matrix_t<T>& a, const matrix_t<T>& b) {
  ...
  for (int filas{1}; filas <= a.get_m(); ++filas) {
    for (int columnas{1}; columnas <= b.get_n(); ++columnas) {
      for (int i{1}; i <= b.get_m(); ++i) {
        v_[pos(filas, columnas)] = v_[pos(filas, columnas)] + a.at(filas, i) * b.at(i, columnas);
      }
    }
  }
}
